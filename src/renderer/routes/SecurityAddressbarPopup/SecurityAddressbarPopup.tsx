...
function formatLastUpdateTime (second?: number) {
  second = parseFloat(second as any);
...
function usePageState () {
  const [ pageState, setPageState ] = useState<ISecurityAddrbarPopupState>({ page: 'entry' });
  ...
  useEffect(() => {
    const dispose = window.rabbyDesktop.ipcRenderer.on('__internal_rpc:security-addressbarpopup:on-show', (evt) => {
  ...
  const switchPageTo = useCallback(async (state: ISecurityAddrbarPopupState) => {
    switch (state.page) {
      case 'entry': {
        window.rabbyDesktop.ipcRenderer.sendMessage('__internal_rpc:security-addressbarpopup:switch-pageview', state);
        break;
      }
      case 'detail-item': {
        window.rabbyDesktop.ipcRenderer.sendMessage('__internal_rpc:security-addressbarpopup:switch-pageview', state);
  ...
  useEffect(() => {
    window.rabbyDesktop.ipcRenderer.on('__internal_rpc:security-addressbarpopup:switch-pageview', ({ state }) => {
  ...
  return {
    ...
    ...pageState.page === 'detail-item' && {
      page: pageState.page,
      focusingDetailed: pageState.page === 'detail-item' ? pageState.focusingItem : null,
    },
    switchPageTo
  }
}
    ...
    <div className={classNames(
      ...
      {type === 'checkHttps' && (
        <main>
          <div className={styles.markedInfo}>
            ...
            <div className={classNames(styles.summaryText, `j-summary-${checkResult.checkHttps.level}`)}>
          ...
          <div className={classNames(styles.details, `details-${type}`)}>
            ...
            <p className={styles.paragraph}>
              HTTPS certificate encrypts your information shared with the dapp, and provides secure communication with the official server. An expired or invalid certificate may lead to a leak of private information.
            </p>
          </div>
        </main>
      )}
      ...
      {type === 'checkLatestUpdate' && (
        <main>
          <div className={styles.markedInfo}>
            <div className={styles.centerIcon}>
              <RcIconAddrbarPopupCheckItemLastUpdate className={styles.iconSvg} />
              <RcIconAddrbarPopupShieldOk className={styles.levelIconSvg} />
            </div>
            <div className={classNames(styles.summaryText, `j-summary-${checkResult.checkLatestUpdate.level}`)}>
              <p className={styles.text}>
                {!checkResult.checkLatestUpdate.latestChangedItemIn24Hr ? 'Updated more than 24 hours' : 'Updated less than 24 hours'}
              </p>
              <div className={styles.lastUpdate}>
                Last updated: <time>{formatLastUpdateTime(checkResult.checkLatestUpdate.latestItem?.create_at)}</time>
              </div>
          ...
          <div className={classNames(styles.details, `details-${type}`)}>
            <h4 className={styles.title}>What is the Web page last updated time? </h4>
            <p className={styles.paragraph}>
              It's recommended to use a dapp 24 hours after the updates to prevent the following risks:
              <ul>
    ...
    >
      <header>
        <div className={styles.title}>
          {checkResult.resultLevel === 'ok' && <>Dapp Security Engine found no risks and will continue to scan as you use the dapp</>}
          {checkResult.resultLevel === 'warning' && <>Dapp Security Engine found {checkResult.countWarnings} Warning(s)</>}
        </div>
        <div
        ...
        >
          <div className={styles.left}>
            <div className={styles.JLevelIcon}>
              ...
              {checkResult.checkLatestUpdate.level === 'warning' && <RcIconAddrbarPopupShieldWarning className={styles.iconSvg} />}
              {checkResult.checkLatestUpdate.level === 'ok' && <RcIconAddrbarPopupShieldOk className={styles.iconSvg} />}
            </div>
            <div className={styles.text}>
              <div className={styles.title}>Web page last updated time</div>
              <div className={styles.desc}>
                {checkResult.checkLatestUpdate.timeout ? 'Check Timeout'
                : checkResult.checkLatestUpdate.latestChangedItemIn24Hr ? 'Updated less than 24 hours'
                : 'Updated more than 24 hours'}
              </div>
            </div>
          </div>
        ...
        >
          <div className={styles.left}>
            <div className={styles.JLevelIcon}>
              ...
              {checkResult.checkHttps.level === 'warning' && <RcIconAddrbarPopupShieldWarning className={styles.iconSvg} />}
              {checkResult.checkHttps.level === 'ok' && <RcIconAddrbarPopupShieldOk className={styles.iconSvg} />}
            </div>
            <div className={styles.text}>
              <div className={styles.title}>HTTPS certificate validation</div>
              <div className={styles.desc}>
                {checkResult.checkHttps.timeout ? 'Check Timeout'
                  : checkResult.checkHttps.httpsError ? 'Invalid Https Certifactions'
                  : 'Valid and Verified'}
              </div>
            </div>
          </div>
