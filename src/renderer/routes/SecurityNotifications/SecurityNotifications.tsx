...
const ICON_CLOSE =
  'rabby-internal://assets/icons/security-notifications/icon-close.svg';

const ICON_HEADER_BG_LOGO =
  'rabby-internal://assets/icons/security-notifications/header-bg-logo.svg';

const ICON_SHILED_DEFAULT =
  'rabby-internal://assets/icons/security-notifications/icon-shield-default.svg';
const ICON_SHILED_WARNING =
  'rabby-internal://assets/icons/security-notifications/icon-shield-warning.svg';
const ICON_SHILED_DANGER =
  'rabby-internal://assets/icons/security-notifications/icon-shield-danger.svg';
...
function CardHeader({
  ...
  return (
    <div className={styles.header}>
      <img
        className={styles.headerLogoBg}
        src={ICON_HEADER_BG_LOGO}
        alt="background"
      />
...
function notify(
  ...
  api.open({
    ...
    placement: 'topRight',
    closeIcon: <img src={ICON_CLOSE} className={styles.closeIcon} />,
    ...
    ...(notificationEvent.type === 'full-web3-addr-changed' && {
      message: (
        <CardHeader iconUrl={ICON_SHILED_WARNING}>
          <span className={styles.headerText}>
            Sensitive information in your clipboard has been changed. Please
            compare carefully.
          </span>
        </CardHeader>
      ),
    ...
    ...(notificationEvent.type === 'full-web3-addr-quick-changed' && {
      message: (
        <CardHeader iconUrl={ICON_SHILED_WARNING}>
          <span className={styles.headerText}>
            Found sensitive information changes in clipboard in a second ,
            beware of malicious tampering.
          </span>
        </CardHeader>
      ),
      // btn: <button>Click me!</button>,
...
export default function SecurityNotifications() {
  ...
  useLayoutEffect(() => {
    ...
    const correctCounter = setInterval(() => {
      const allContentNodes = Array.from(
        wrapperNode.querySelectorAll('.ant-notification-notice-content')
      );
  ...
  return (
    <div ref={notificationWrapperRef} className="notification-wrapper">
      <div className={styles.debugContent}>Nothing</div>
      {contextHolder}
    </div>
  );
}
