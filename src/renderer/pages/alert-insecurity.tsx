...
export default function AlertInsecurity() {
  ...
  const webviewRef = useRef<HTMLWebViewElement & import('electron').BrowserWindow>(null);
  useEffect(() => {
  ...
  return (
    <Modal
      ...
      mask
      onCancel={() => {
        window.rabbyDesktop.ipcRenderer.sendMessage('__internal_close-alert-insecure-content')
      }}
      closeIcon={<img className='close-icon' src="rabby-internal://assets/icons/alert-insecurity/icon-close.svg" />}
      footer={null}
    >
      <div className='alert-header'>
        {urlInfo.isExisted ? (
          <div className='inner'>
            This page is in reader mode because it doesn't belong to the current Dapp. You can have full access to it by opening{' '}
            <span className='allowed-url'>
              {origin}
              <img className='icon' style={{ marginLeft: 2 }} src="rabby-internal://assets/icons/alert-insecurity/icon-external-url.svg" />
            </span>
          </div>
        ) : (
          <div className='inner'>
            <img className='icon' style={{ marginRight: 2 }} src="rabby-internal://assets/icons/alert-insecurity/icon-readonly.svg" />
            This page doesn't belong to the current Dapp, so it can only be viewed.
          </div>
        )}
      </div>
